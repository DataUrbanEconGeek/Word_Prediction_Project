...
In this Project we'll use three sets of data to create a word prediction application. The sets are from twitter, various blogs and news sources. They have been provided by SwiftKey through Coursera.com. In this part, Part 1 we'll perform an exploritory analysis of the data. Get an idea of how large the scurces are, if there are any correlations among terms and which words appear most often. In Part 2 we'll create a predictive model and design an application. First lets set the enviroment to the project's directory, then dive in from there. 
...
Lets take a look at some of the information about these files, such as the size of the file and average word count. Here we creat a function that will return a data frame that summarizes the data. 
...
Next we'll grab the samples. We're only going to use an extreamly small percentage of the data. Using roughly 2% of the data is small enough to make our calculations. 
...
blogs_cloud <- wordcloud(frequency_df$word[1:50], frequency_df$blogs[1:50], 
          scale = c(5,1), min.freq = 202, 
          colors = blogs_pal, rot.per=0.35, use.r.layout=FALSE, random.order = FALSE)
...
As you can see from the table above the word "the" is used the most often in all the sets. This dose not surprise me. 

Now if we plot the sets against each other, we can see that there is a strong correlation amungst words in all the sets. Meaning that words that often appear in one set will also often appear in another set. The correlation coefficient further backs this up.  
...
plot1 <- ggplot(data = frequency_df, aes(x = twitter, y = blogs)) + 
        ...
        annotate("text", x=Inf, y = Inf,
                                label = paste0("cor: ",
                                        round(cor(frequency_df$twitter, frequency_df$blogs), 2)),
                                vjust=1, hjust=1, col = "#F8766D") +
        labs(title = "Twitter vs. Blogs Frequency of Words")
plot2 <- ggplot(data = frequency_df, aes(x = twitter, y = news)) +
        geom_point() +
        annotate("text", x=Inf, y = Inf,
                                label = paste0("cor: ",
                                        round(cor(frequency_df$twitter, frequency_df$news), 2)),
                                vjust=1, hjust=1, col = "#F8766D") +
        labs(title = "Twitter vs. News Frequency of Words")
plot3 <- ggplot(data = frequency_df, aes(x = news, y = blogs)) +
        geom_point() + 
        annotate("text", x=Inf, y = Inf,
                                label = paste0("cor: ",
                                        round(cor(frequency_df$news, frequency_df$blogs), 2)),
                                vjust=1, hjust=1, col = "#F8766D") +
        labs(title = "News vs. Blogs Frequency of Words")
